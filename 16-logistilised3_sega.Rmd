...
```{r}
lapply(dat[,c("LM_mobiilsus", "LM_komplekssus", "tegusõna_klass", "murderühm")], function(x) names(which.max(table(x))))
```
...
plot_model(m5.glm, show.values = T, value.offset = 0.3, 
           sort.est = T, p.adjust = "holm", 
           title = "Baaskontekst:\nLM mobiilsus [staatiline], LM komplekssus [lihtsõna], LM pikkus log [0 ehk 1 silp], murderühm [Kesk]", 
           wrap.title = 100) +
...
Kuna logistilises regressioonis ei pakuta meile R-ruutu, mille põhjal mudeli headust ja täpsust hinnata, tuleb kasutada nt klassifitseerimistäpsust ja C-indeksit/AUC-d. Nendest pisut allpool.
...
head(m5.glm$linear.predictors) # logaritmitud šansid
...
FPR <- klass_tabel["ade", "peal"]/sum(klass_tabel["ade",]) # kui palju ade-konstruktsioonde mudel peal-konstruktsioonideks pidas?
...
m5.lrm <- lrm(cx ~ LM_mobiilsus + LM_komplekssus + tegusõna_klass + LM_pikkus_log*murderühm, data = dat2)
...
m5.lrm$stats["C"]
```
C-indeksi arvutamiseks võib kasutada ka teisi pakette.

```{r, warning = F, message = F}
library(Hmisc)
somers2(m5.glm$fitted.values, ifelse(dat2$cx == "peal", 1, 0))["C"]

# või
library(pROC)
auc(dat2$cx, m5.glm$fitted.values)
```
...
par(mfrow = c(3, 2))
plot(xseq+0.25, densities, col="darkred", xlab="", ylab="", type="l", lwd=2, cex=2, xlim = c(0, 1), main = "Kahe klassi ennustatud\ntõenäosuste jaotus")
...
plot(xseq+0.5, densities, col="darkred", xlab="", ylab="", type="l", lwd=2, cex=2, xlim = c(0, 1), main = "Kahe klassi ennustatud\ntõenäosuste jaotus")
...
plot(xseq+0.75, densities, col="darkred", xlab="", ylab="", type="l", lwd=2, cex=2, xlim = c(0, 1), main = "Kahe klassi ennustatud\ntõenäosuste jaotus")
...
Kõige produktiivsematelt keelejuhtidelt on andmestikus tervelt paarkond vaatlust, samas kui keskmiselt on kõneleja kohta 4-5 vaatlust. Seetõttu on esimene regressiooni eeldus tegelikult rikutud.
...
```
`performance` pakett pakub veel mitmeid funktsioone mudeli diagnostika tegemiseks, täpsuse leidmiseks jpm-ks.  
...
```{r}
m5.lrm <- update(m5.lrm, x = TRUE, y = TRUE)
...
Kui nimetatud statistikute puhul jäävad `optimism`-tulba väärtused alla 0.05, siis peaks kõik korras olema.
...
# C-indeks/AUC
somers2(enn, ifelse(test$cx == "peal", 1, 0))["C"]
```
...
```{r}
tab_model(m5.glm, transform = NULL, show.ci = F, show.se = T, show.r2 = F, show.loglik = T, show.stat = T, show.dev = T, show.reflvl = T, p.adjust = "holm", title = "(AUC = 0.746, null deviance = 2002)", string.stat = "z")
```
...
m1.glmer <- glmer(cx ~ LM_mobiilsus + LM_komplekssus + tegusõna_klass + LM_pikkus_log * murderühm + (1|kõneleja), data = dat2, family = "binomial", control = glmerControl(optimizer= "bobyqa"))
summary(m1.glmer)
```
...
Tundub, et AIC läheb väiksemaks ning parameetrite hinnangutes ja statistilises olulises erilisi muutuseid ei toimu. Võib-olla võiks kaaluda kohafraasi pikkuse ja murderühma interaktsiooni väljajätmist, kuna seal kaoksid ilmselt pärast Bonferroni või Holmi-Bonferroni parandusi statistiliselt olulised erinevused. Jätame selle praegu siiski mudelisse alles.    
...
```{r}
library(Hmisc)
somers2(enn, ifelse(dat2$cx == "peal", 1, 0))["C"]
```
Mudeli headuse näitajad on läinud oluliselt paremaks!  


Vaatame ka juhuslike mõjude jaotumist funktsiooniga `ranef()`. Tegemist on juhuslike vabaliikmete kohandustega ehk vahega mudeli üldistatud vabaliikme ning iga indiviidi jaoks ennustatud vabaliikme vahel. Teisisõnu näitavad need väärtused, kui palju mingi informant *peal*-konstruktsiooni kasutamises keskmisest *peal*-kasutamisest erineb.
```{r, message = FALSE}
juh <- ranef(m1.glmer, condVar = TRUE)
head(juh$kõneleja)
mean(juh$kõneleja$`(Intercept)`) # keskmine on 0 lähedal
median(juh$kõneleja$`(Intercept)`) # mediaan on 0 lähedal
...
Tundub, et mudelis on olulisem murdeala kui juhuslik kõneleja ise. See tähendab, et kohandused vabaliikmes ei tee enam sama asja, mida juba murderühm fikseeritud mõjuna.  
...
Juhuslikke kaldeid tunnuse `kõneleja` põhjalt on keeruline lisada, kuna meil on `kõneleja`l väga palju tasemeid, mudelis mitu mitme tasemega fikseeritud tunnust ning esineb väga harvu tunnusekombinatsioone. Võiksime ju näiteks oletada, et kuna informandid on oma murdetaustast ja üldkeelest erineval määral mõjutatud, on ka murdeala mõju keelekasutusele varieeruv. Kui üritame aga seda juhusliku kaldena mudeldada, saame teate, et mudel ei suuda parameetreid hinnata, sest meil pole selleks lihtsalt piisavalt palju andmeid. Mudel küll tehakse ära ja saame selle väljundit vaadata, ent me ei saa olla päris kindlad selle täpsuses.

```{r}
# mudeli tegemine võib võtta natuke aega
m2.glmer <- glmer(cx ~ LM_mobiilsus + LM_komplekssus + tegusõna_klass + LM_pikkus_log * murderühm + (1 + murderühm|kõneleja), data = dat2, family = "binomial", control = glmerControl(optimizer = "bobyqa"))
summary(m2.glmer)
```
...
Küll aga võiksime proovida lisada mudelisse juhusliku mõjuna ka kohasõna lemma `LM_lemma`, kuna käände- või *peal*-konstruktsiooni kasutus võib sõltuda osaliselt ka sellest, mis sõnad on meie andmestikku sattunud.  

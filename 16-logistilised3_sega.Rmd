...
- Nende vaatluste hulka, kus ennustatud tõenäosus on > 0.5 JA tegelik väärtus andmestikus on `cx = "peal"`. Neid nimetatakse ka **tõeselt positiivseteks** juhtudeks (*true positives, TP*).  
- Nende vaatluste hulka, kus ennustatud tõenäosus on < 0.5 JA tegelik väärtus andmestikus on `cx = "ade"`. Neid nimetatakse **tõeselt negatiivseteks** juhtudeks (*true negatives, TN*).  
...
# tõeselt positiivsed, valepositiivsed,
# tõeselt negatiivsed ja valenegatiivsed
# juhud
...
Lisaks tasub uurida ka, kui hästi mudel mingite vaatluste väärtusi ennustab, ehk uurida mudeli jääke.  
...
### Lineaarne suhe šansi logaritmi ja arvuliste (!) seletavate tunnuste vahel
...
Logistilise regressiooni mudeli jäägid (*deviance residuals*) näitavad erinevust vaatlustele ennustatud tõenäosuse (0-1) ja nende tõelise väärtuse (0 või 1) vahel. Jääkide arvutamiseks kasutatakse valemit `sqrt(-2*log(ennustatud))`, kui vaatluse väärtus on 1 (siin: "peale") ja valemit `-sqrt(-2*log(1-ennustatud))`, kui vaatluse väärtus on 0 (siin: "ade"). Näiteks:  
...
```{r}
# Võrdleme mudeleid
tab_model(m5.glm, m1.glmer, show.aic = T, use.viewer = FALSE)
```

...
```{r}
lapply(dat[,c("LM_mobiilsus", "LM_komplekssus", "tegusõna_klass", "murderühm")], # iga tulba/tunnuse kohta neljast
       function(x) names(which.max(table(x)))) # leia selle tunnuse mood
```

Tüüpiline kontekst oleks niisiis midagi sellist nagu *auto __on__ __tee__l / __tee__ peal*.  
...
plot_model(m5.glm, 
           ...
           p.adjust = "holm", # korrigeeri p-väärtuseid
...
plot_model(m5.glm, type = "pred") -> preds
preds$LM_mobiilsus
preds$LM_komplekssus
preds$tegusõna_klass
```
...
head(m5.glm$y) # esimesed 6 vaaltust on kõik 0
```

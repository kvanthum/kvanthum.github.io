...
```{r, eval=F}
# ajaloo eriala keskmine; tabel[read, tulp]
mean(kysimustik[kysimustik$oppekava == "ajalugu", "kaua_opid"])
# eesti ja soomeugri keskmine; tabel$tulp[read]
...
for(i in levels(kysimustik$oppekava)){
    ...
    print(paste0(i, " üliõpilased on keskmiselt õppinud ", keskmine, " aastat."))
}
```
...
Ja sama asja saab `tidyverse` paketiga. `tidyverse` paketist tuleb juttu ülejärgmises praktikumis, mistõttu siin pikemalt selle süntaksil ei peatu. R-i väljundis näeb tabel inetu välja, pakett `flextable` abil saab tabeli vormistada vahejoontega tabuleeritud tabelina.
...
Tihedusgraafik näitab suhteliselt sarnaselt histogrammile andmete jaotust, aga absoluutsageduste asemel on y-teljel tõenäosuslik sagedus (vahemikus 0--lõpmatus).

```{r}
plot(density(kysimustik$synniaasta))
```
...
- `density()` - arvutab andmestiku tõenäosustiheduse. Tihedusgraafiku joonistamiseks pane sellele ümber `plot()`.

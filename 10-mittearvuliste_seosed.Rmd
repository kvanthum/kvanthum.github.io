...
**Sõltumatuse testis** (*independence*) uurime kahe tunnuse vahelist seost samas valimis, **homogeensuse testis** (*homogeneity*) aga uurime, kas ühe tunnuse kategooriate proportsioonid on kahes sõltumatus valimis erinevad. See eristus võib aga kohati olla keeruline, kuna alati pole selge, mis hetkest peaks rääkima eraldi valimitest/gruppidest. Testi tegemine käib mõlemal juhul samamoodi, ent erinevused tulenevad valimi moodustamisest ja testi tõlgendamisest.
...
tab <- matrix(c(43, 77, 377, 343), # koosesinemise sagedused
              ncol = 2, # mitmesse tulpa sagedused jaotada?
...
Kui esitatud näide sind ei kõneta, võid teha tabeli ka mõne muu uurimuse põhjal, kus risttabelites sagedusi on raporteeritud.   
...
<center>
<p>Annika Kängsepp (2024). [Käändevormide varieerumine ja seda mõjutavad tegurid kirjalikus keeles indefiniitpronoomeni *keegi* näitel](https://keeljakirjandus.ee/wp-content/uploads/2024/11/A.KANGSEPP.pdf). Keel ja Kirjandus 11, 1016-1037.</p>
<p>Normingupäraste (*NP*, nt *kellelgi*) ja norminguvastaste (*NV*, nt *kellegil*) käändevormide jagunemine vastavalt teksti žanrile</p>
![](imgs/10_kangsepp_artikkel.png){width=60%}
</center>

```{r, eval = F}
tab <- matrix(c(227,184,239,551,434,165),
              ncol = 3,
              dimnames = list(c("NP", "NV"),
                              c("Blogid", "Foorumid", "Perioodika")))
```
...
<center>
<p>Annaliisa Mäletjärv (2021). [Varjatud emotsioonide äratundmine näoväljendustest](https://sisu.ut.ee/wp-content/uploads/sites/561/maletjarv_annaliisa_uurimistoo.pdf). Uurimistöö.</p>
<p>Videoklippidele antud hinnangud väljendatud emotsioonide kohta (ridades) ning tegelikud (uurija poolt määratud) emotsioonid (tulpades)</p>
![](imgs/10_maletjarv_uurimistoo.PNG){width=60%}
<!-- Veebipõhine katse, kus 27 vastajat hindasid videoklippide põhjal, millise emotsiooniga on tegemist-->
</center>

```{r, eval = F}
tab <- matrix(c(301,489,345,397,373,285,
                287,583,382,343,300,302,
                264,564,344,378,310,332),
              ncol = 3,
              dimnames = list(c("Rõõm", "Kurbus", "Vastikus", "Hirm", "Üllatus", "Viha"),
                              c("Rõõm", "Vastikus", "Hirm")))
```
...
<center>
<p>Pille Markov (2019). [Intellektuaalne ja visuaalne realism laste joonistustes](https://dspace.ut.ee/bitstreams/ff20444f-d980-44d2-9288-990b502aafbb/download). Uurimistöö.</p>
<p>Laste vanuserühmad ning valitud joonistamisstrateegia laste joonistuskatses</p>
![](imgs/10_markov_uurimistoo.PNG){width=60%}
<!-- Katse, milles paluti 5-8-aastastel lastel joonistada nende ees oleval pildil olevat inimest/inimesi, kusjuures mingitel fotodel olid inimesed seljaga diivani taga; osal fotodest oli näha kaht inimest, kellest üks seisab seljaga diivani taga, teine näoga pildistaja suunas; kolmas rühm fotosid kuvasid inimest täispikkuses. Intellektuaalne realism (laps teab midagi asjade kohta ja joonistab neid), visuaalne realism (jäetakse välja asju, mida teatakse, objektid hakkavad kattuma -> kolmemõõtmelisus) -->
</center>
```{r, eval = F}
tab <- matrix(c(14,8,23,31),
              ncol = 2,
              dimnames = list(c("Noorem", "Vanem"),
                              c("Intellektuaalne realism", "Visuaalne realism")))
```
<br>
<br>
...
<center>
<p>Kai Tiitsaar (2010). [Hugo Treffneri Gümnaasiumi õpilaste käelisus](https://akadeemiake.ee/wp-content/uploads/2011/09/akadeemiake08.pdf#page=29). Akadeemiake 1(8), 29-50.</p>
<p>Käelisus poiste ja tüdrukute hulgas</p>
![](imgs/10_tiitsaar_kaelisus.PNG)
</center>
```{r, eval = F}
tab <- matrix(c(15,18,199,13,7,241),
              ncol = 2,
              dimnames = list(c("Vasakukäelised", "Mõlemakäelised", "Paremakäelised"),
                              c("Poisid", "Tüdrukud")))
```
<br>
<br>
...
df <- tab %>% # võtame table-tüüpi objekti
  ...
  rownames_to_column(var = "Vorm") %>% # paneme reanimed tulpa "Vorm"
...
### Tegelikud ja oodatud sagedused
...
```{r}
(tab_margins <- addmargins(tab)) # lisame tabelile rea- ja veerusummad
```

```{r}
# Näiteks lühikese sisseütlevaga pärisnimede (kuhu? - Tartu) teoreetiline sagedus oleks
# (reasumma*veerusumma/kogusumma)
tab_margins["Lühike","Sum"]*tab_margins["Sum","Pärisnimi"]/tab_margins["Sum","Sum"]
```
...
Kui harjutuses 1 vaatlesime esmalt, kas kvantitatiivsete meetodite **kogemuse omamine** sõltub lemmikjoogist, ning seejärel, kas **kogemuse omamine** sõltub programmeerimisoskusest, siis testime tegelikult samade andmete põhjal mitut hüpoteesi ning lubame iga testi puhul nullhüpoteesi ümberlükkamisel teatud eksimismäära (nt 0,05 = 5-protsendiline eksimismäär). Mitme testi tegemisel need eksimismäärad aga kumuleeruvad. Seda nimetatakse **mitmese testimise probleemiks** (*multiple testing*, *multiple comparisons*): mida rohkem teste või võrdlusi teha, seda rohkem kasvab võimalus leida vähemalt üht statistiliselt olulist seost pelgalt juhuse teel.  
  
Probleemi aitab kvantifitseerida nt nn **katseviisilise vea mõõdik** (*Family-Wise Error Rate*, FWER), mille arvutamiseks on vaja teada olulisuse nivood (*α*) ning testide/võrdluste arvu (*m*):  

$$
FWER = 1 - (1-α)^m
$$
Näiteks kahe testi tegemisel oleks FWER `1-(1-0.05)^2=0.0975`, kolme puhul `1-(1-0.05)^3=0.142625`, kahekümne testi tegemisel aga juba `1-(1-0.05)^20=0.6415`. See tähendab, et tõenäosus teha vähemalt ühe testi/võrdluse puhul nn esimest tüüpi viga (lükkame nullhüpoteesi ümber, ehkki see tegelikult kehtib) oleks vastavalt 9,75%, 14,2% ja 64,15%.    

Mitmese testimise probleemi aitab lahendada kas α ja/või p-väärtuste kohandamine võrdluste arvuga vastavaks.   

**Olulisuse nivoo** kohandamisel nn **Bonferroni parandusega** jagatakse olulisuse nivoo (nt 0,05) läbi testide/võrdluste arvuga (*m*) ning iga testi/võrdluse p-väärtus peab jääma alla kohandatud olulisuse nivoo, selleks et võiksime väita, et tegu on statistiliselt olulise seosega. Näiteks kui teeme kaks testi, siis peab testide p-väärtus olema väiksem kui `0.05/2=0.025`; kui teeme 20 testi, siis peab p-väärtus olema väiksem kui `0.05/20=0.0025`.  

```{r, eval = F}
load("data/kysimustik_2026.RData")
chisq.test(kysimustik$kogemused_kvant, kysimustik$lemmikjook) # > 0.025, ei ole oluline
chisq.test(kysimustik$kogemused_kvant, kysimustik$programmeerimisoskus) # < 0.025, on oluline
```

Bonferroni parandust peetakse vahel liiga konservatiivseks. Seetõttu on sagedamini kasutusel **Holmi-Bonferroni parandus**, mis ei kohanda olulisuse nivood kõikidele testidele ühtmoodi, vaid   

- järjestab kõikide testide p-väärtused kasvavas järjekorras,  
- jagab olulisuse nivoo *α* läbi järjest leebemalt:  
  + kõige väiksema p-väärtusega testi jaoks: tehtavate võrdluste/testide arvuga (*m*), *α = α/m*,  
  + järgmiseks kõige väiksema p-väärtusega testi jaoks: *α = α/(m-1)*,   
  + järgmiseks kõige väiksema p-väärtusega testi jaoks: *α = α/(m-2)*,  
  + jne, kuni kõige suurema p-väärtusega testi jaoks jagame olulisuse nivoo läbi 1-ga: *α = α/1*.  

```{r}
library(tidyverse)
testide_p <- c(0.01, 0.0645, 0.045, 0.235, 0.4363, 0.06, 0.0003)
a_kohandused <- 0.05/length(testide_p):1
data.frame(testide_p = sort(testide_p), 
           a_kohandus = a_kohandused) %>% 
  mutate(oluline = ifelse(testide_p < a_kohandus, TRUE, FALSE))
```


```{r, eval = F}
load("data/kysimustik_2026.RData")
# teiseks kõige väiksem p-väärtus: α = α/(m-1) = 0.05/(2-2) = 0.05
chisq.test(kysimustik$kogemused_kvant, kysimustik$lemmikjook) # > 0.05, ei ole oluline

# kõige väiksem p-väärtus: α = α/m = 0.05/2 = 0.025
chisq.test(kysimustik$kogemused_kvant, kysimustik$programmeerimisoskus) # < 0.025, on oluline
```


R-is võime aga **kohandada ka p-väärtusi** otse, kasutades nt funktsiooni `p.adjust()`.     

```{r}
p.adjust(p = c(0.2354, 0.001185, 0.049), method = "holm", n = 3)
```

<br>

Lisaks Bonferroni ja Holmi-Bonferroni parandusele on olemas ka teisi (nt Scheffe test, Tukey test). On ka selliseid meetodeid, mis ei lähtu mitte katseviisilisest veast (FWER), vaid hoopis nn **valeavastusmäärast** (*False Discovery Rate*, FDR), mis näitab, kui suur on statistiliselt oluliste seoste hulgas selliste juhtude oodatav osakaal, milles hülgame nullhüpoteesi ekslikult.  
...
# Jääkide ruutude summa ongi hii-ruut-statistik
sum(t$residuals^2)
```
...
Kui olulisuse nivoo on 0.05, siis on statistiliselt olulised need jäägid, mis on suuremad kui 1.96 või väiksemad kui -1.96 (tihti ümardatakse ka 2 ja -2). Vastava kriitilise väärtuse võib leida nt funktsiooniga `qnorm(1-olulisuse_nivoo/2)`. Need jäägid näitavad, millised on need risttabeli lahtrid, mis panustavad hii-ruut-testi statistiliselt olulisse tulemusse enim ning milles erinevus oodatud ja tegelike sageduste vahel on kõige märgatavam.    
...
```{r}
report::report_text(chisq.test(tab, correct = F))
```
...
Ka hii-ruut-testi jaoks võib arvutada välja testi võimsuse, mis ütleb, kui suure tõenäosusega leiame sellise valimi suuruse, vabadusastmete arvu, mõju suuruse ja olulisuse nivoo korral olulise seose juhul, kui see tõesti olemas on. Testi võimsust tasub kontrollida eriti siis, kui saad testiga statistiliselt ebaolulise tulemuse.  
...
Võimsuse analüüsiga saab ka otsustada, kui palju andmeid on vaja selleks, et leida teatud olulisuse nivool ning kindla võimsusega mingi soovitud suurusega mõju.  
...
pwr.chisq.test(w = w, df = df, power = power, sig.level = 0.05)
...
Kuna tugevamat mõju (*w*) on lihtsam andmetest tuvastada, on selleks vaja ka vähem vaatlusi. Väikese (aga olulise) mõju tuvastamiseks on aga jällegi vaja rohkem vaatlusi.  
...
Kontrolli seejärel, kas äkki tuli küsimustiku andmete põhjal seos lemmikjoogi ja kvantitatiivsete meetodite kogemuse vahel ebaoluline seetõttu, et testi võimsus polnud selle andmehulga jaoks piisav.  
...
Järgmisel korral läheme kahe tunnuse vaheliste seoste hindamise juurest regressioonimudelite juurde, mis võimaldavad ühe tunnuse muutumist kirjeldada enamate tunnuste kaudu. **NB!** Järgmine praktikum alles ülejärgmisel nädalal (30.03)!

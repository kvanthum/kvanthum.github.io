...
```{r ylla-naited, echo=F, message=F}
library(tidyverse)
library(flextable)
...
```
  ...
  - Stiimulitena varieeriti 12 erinevat **mis**-küsimust ja 12 **mida**-küsimust. *Mis*-küsimused olid kõik struktuuriga *Mis NOOMEN see on?* ning *mida*-küsimused *Mida sa VERB?*. 
  - Kõik küsimused esitati kord info- ja kord üllatuskontekstis. Küsimuse tüüp on kodeeritud anmdestikus __ISQ__ -- *Information-Seeking Question* e infoküsimus ja __SQ__ -- *Surprise Question* e üllatusküsimus.
  - Seega kokku oli katses 2 tüüpi x 2 küsisõna x 12 erinevat lauset = 48 stiimulit. 
  - Katseisikuid oli kokku 21 (kõik naised), seega kokku on andmestikus 21 x 48 = 1008 vastust. 
  - Kõva häälega loetud küsimused salvestati ja mõõdeti lausungi kestust ja põhitooni.

```{r ylla-joonis, warning=FALSE, message=F, echo=F}
load("data/Est_SQ-ISQ_dataset_JLing.Rda")

ylla_f0_confint <- data.frame()
for(i in levels(dat$tyyp1)){
    for(j in levels(dat$kysisona)){
    ylla_f0_confint <- rbind(ylla_f0_confint,    
                             apply(dat[dat$tyyp1 == i & dat$kysisona == j,paste0("f0_", 0:95)], 2, gmodels::ci, na.rm=T) %>% 
                                 t() %>% data.frame() %>% 
                                 add_rownames(., var="row") %>% 
                                 mutate(t = as.numeric(gsub(".*_(\\d)","\\1",row)), aeg = t/100*mean(dat$phr_dur[dat$tyyp1==i & dat$kysisona ==j]), tyyp1 = i, kysisona = j)
    )
    }}
dat %>% 
    group_by(tyyp1, kysisona) %>% 
    summarise(w1 = 0, 
              w2 = mean(w_start_2-w_start_1), 
              w3 = mean(w_start_3-w_start_1),
              w4 = mean(w_start_4-w_start_1)) %>% 
    pivot_longer(paste0("w",1:4), names_to = "sona", values_to = "aeg") %>% 
    mutate(lev = as.numeric(tyyp1)) -> tmp
tmp$sona <- rep(c("mis", "NOOMEN","see","on", "mida", "x", "sa","VERB"),2)


(joonis <-  ylla_f0_confint %>% 
    ggplot(aes(x = aeg, colour = tyyp1, fill = tyyp1))+
    geom_line(aes(y = Estimate))+
    geom_ribbon(aes(ymin = CI.lower, ymax = CI.upper), alpha = 0.5)+
    geom_text(data = tmp, aes(x = aeg, y = 150+8^(lev-1), label = sona), size = 6, hjust = 0, show.legend = FALSE)+
    facet_grid( "kysisona")+
    labs(y = "Põhitoon (Hz)", x = "Aeg (s)", colour = "Küsimuse tüüp",  fill = "Küsimuse tüüp"))
```
...
**Populatsiooni** ehk **üldkogumi** moodustavad uurimisobjekti kõik esindajad. Enamasti on populatsioon nii suur ja raskesti kättesaadav, et ei ole võimalik uurida kogu populatsiooni. Näiteks kui uurime inimkäitumist üldiselt, siis populatsioon on kogu inimkond; kui uurime eesti keelt, siis on populatsiooniks kõik eesti keele kõnelejad ja kõik, mis nad iganes räägivad, või kõik eesti keeles kirjutatud tekstid. 

Sõltuvalt uurimisküsimustest võib muidugi ka populatsioon olla hoomatavam: näiteks kui uurime Anton Hansen Tammsaare loomingut, siis kogu populatsiooni moodustavad kõik tema teosed (mida praeguseks on lõplik hulk).
...
- Plussid:
    + täpne info kõigi populatsiooni esindajate kohta.
- Miinused:
    + töömahukas (sageli mastaabi tõttu teostamatu),
    + kallis,
...
- Näiteks kui uurida leibkondade elujärge, peavad olema valimisse kaasatud kõik leibkonnatüübid. Või kui näiteks uurida eesti kirjandust, peavad olema valimis erinevad kirjanikud ja erinevad kirjandusžanrid.
...
- Peab kuidagi tagama, et kõigil populatsiooni liikmetel oleks võrdne võimalus valimisse sattuda. Rahvastiku uurimisel on näiteks võimalik rahvastikuregistrist küsida juhuslik hulk inimeste kontaktandmeid.
...
- Näiteks rahvaloenduse andmetel teame, kui palju on erineva haridustasemega mehi ja naisi Eesti eri piirkondades ja valimi moodustame nii, et nende tunnuste jaotus oleks sarnane.

**Esindusvalim**: valim koostatakse nii, et erinevate võimalike tunnuste proportsioonid on võrdsed.

- Näiteks võetakse valimisse sama arv mehi ja naisi igast vanuserühmast, igast uuritava üldkogumi piirkonnast jne;
...
Ühe- või kahepoolne hüpotees sõltub sisuka ehk alternatiivse hüpoteesi H1 sõnastusest: kas kontrollitakse ainult erinevusi või on erinevustel eeldatud kindel suund.

**Kahepoolne hüpotees** (*two-sided*, *two-tailed*) testib seda, kas (ühe rühma korral) rühma keskmine on erinev mingist konstandist (nt populatsiooni keskmine) või (kahe rühma korral) kas ühe rühma keskmine on erinev teise rühma keskmisest.

- H0: keskmine = a VÕI keskmine1 = keskmine2
- H1: keskmine ≠ a VÕI keskmine1 ≠ keskmine2

Kahepoolne hüpotees on üldisem, lükates nullhüpoteesi igal juhul ümber siis, kui rühmad on erinevad, ükskõik kas üks on teisest suurem või väiksem.

**Ühepoolne hüpotees** (*one-sided*, *one-tailed*) testib seda, kas (ühe rühma korral) rühma keskmine on suurem kui konstant või kas (kahe rühma korral) ühe rühnma keskmine on suurem kui teise rühma keskmine. Võimalust, et rühma keskmine on võrreldavast väiksem, ei kontrollita.

- H0: keskmine <= a VÕI keskmine1 <= keskmine2
- H1: keskmine > a VÕI keskmine1 > keskmine2

Ühepoolne hüpotees võib olla teistpidi, et rühma keskmine on väiksem kui konstant või teise rühma keskmine:

- H0: keskmine >= a VÕI keskmine1 >= keskmine2
- H1: keskmine < a VÕI keskmine1 < keskmine2

Seega ühepoolset hüpoteesi võiks rakendada juhul, kui teooriast on kindlasti teada, milline see erinevus peaks olema ja vastupidine võimalus ei ole võimalik või ei ole oluline.

...
valimised$amet %>% # võta valimiste andmestikust tulp "amet"
  ...
  gsub("[[:punct:]]", "", .) %>% # kustuta sõnade küljest kirjavahemärgid (= asenda mittemillegagi)
...
valimised$amet %>% 
  str_split(" ") %>% 
  lapply(., function(sonad) sonad[length(sonad)]) %>% # igast ametinimetusest võta ainult viimane sõnavorm
  unlist() %>% 
  tolower() %>%
  gsub("[[:punct:]]", "", .) %>% 
  table() %>%
...
# ei tööta
valimised$nimi |>
  gsub("^([^ ]+?) .*$", "\\1", .) |> # punkt viitab eelmise käsu väljundile (vektorile valimised$nimi)
  unique() |>
  length()
```
...
kysimustik %>%
    filter(lemmikjook == "Kohv") %>%
    count(aasta) %>% # risttabeli asemel loe aastate read kokku funktsiooniga count()
    filter(n == max(n)) %>% # filteeri ainult suurima sagedusega rida
    pull(aasta) %>% # võta välja ainult aasta tulba väärtus
    as.character() # muuda faktor tekstivektoriks
```
...
# või vastupidi järjekorras
# valimised <- valimised %>%
#   mutate(hääli_paberil = hääli-kokku-e.hääli)
```
...
Näita joonisel, milline oli kandidaatide vanuste jaotus erakondades ning kas vanus mõjutas kuidagi ka häältesaaki.  
...
Samad arvandmed saaksime **tabelita kujul** kätte ka ilma `summarise()`-funktsioonita, ainult baaspaketti kasutades.

...
*Tidyverse*'i pakette ühendab sarnane kindlas järjekorras asjade tegemise loogika. Üks paketi oluline omadus on võimalus kasutada nn **torusid** ehk *pipe*'e (sümbol `%>%`), mille funktsioon tuleb tegelikult `tidyverse`'iga koos installitavast ja laaditavast paketist nimega `magrittr`. Torud lubavad tõsta R-i n-ö pesastatud funktsioonidest funktsioone välja nende käivitamise järjekorra alusel, suunates iga eelmise käsu väljundi järgmisse käsku.  
...
```{r, eval = F}
# Leia ametinimetustes esinevad 50 sagedamat sõna
# baaspakett
head(sort(table(gsub("[[:punct:]]", "", tolower(unlist(strsplit(valimised$amet, " "))))), decreasing = T), 50)

# tidyverse
valimised$amet %>% # võta valimiste andmestikust tulp "amet"
  strsplit(" ") %>% # lõika ameti kirjeldused tühikute koha pealt sõnadeks
...
Laadi RStudiosse kursuse küsimustiku andmestik. Leia torusid kasutades, millisel aastal on olnud kursusel kõige rohkem kohvijoojaid. Ärme esialgu sellepärast muretseme, kui palju osalejaid mingil aastal üldse kursuselt läbi käis.  
...
valimised %>%
  ...
  ggplot(aes(y = reorder(nimekiri, paberhäälte_protsent), # järjesta nimekirjad protsendi tunnuse alusel 
...
valimised %>%
  ...
  mutate(meil = ifelse(test = grepl("@", kontakt), # kui kontaktandmetes on @-sümbol,
                       yes = strsplit(kontakt, "; ") %>% # siis lõika kontaktandmete tekst semikooloni ja tühiku järjendi koha pealt tükkideks,
...
# see töötab samamoodi nagu valimised %>% count(), ainult et siin saame määrata ka tulbanime, millesse sagedused lisatakse
...
valimised %>%
  summarise(eesnimesid = gsub("^([^ ]+?) .*$", "\\1", nimi) %>% unique() %>% length(),
            perenimesid = gsub("^.* ([^ ]+)$", "\\1", nimi) %>% unique() %>% length())
```

Samad andmed saaksime tabelita kujul kätte ka ilma `summarise()`-funktsioonita.
...
```{r, eval = F}
# Kandidaatide arv valimisnimekirjades
valimised %>%
  group_by(nimekiri) %>%
  summarise(kandidaate_kokku = n())
# kuna loeme siin jällegi lihtsalt vaatlusi kokku,
# siis sama asja teevad valimised %>% count(nimekiri)
# ja valimised %>% group_by(nimekiri) %>% tally()
...
Vahe on selles, et `tapply()` käsu väljund on vektor või maatriks, aga `group_by()`+`summarise()` annavad väljundiks nn pika andmetabeli, kus iga grupeerivate tunnuste kombinatsioon on omaette real (nt Eesti Keskerakond + M, Eesti Keskerakond + N, Isamaa Erakond + M, Isamaa Erakond + N jne) ning kuhu võib lisada ka muid kokkuvõtteid.  
...
Funktsioonid `group_by()`+`summarise()`, `group_by()`+`tally()` ja `count()` ja  teevad väärtusi kokku lugedes sisuliselt sama asja, ainult et `count()` ei jäta andmeid kunagi grupeerituks.    

```{r, eval = F}
valimised %>% group_by(nimekiri) %>% summarise(n = n())
valimised %>% group_by(nimekiri) %>% tally()
valimised %>% count(nimekiri)
```

```{r, eval = F}
valimised %>% group_by(nimekiri, sugu) %>% summarise(n = n()) # andmed jäävad nimekirja järgi grupeerituks
valimised %>% group_by(nimekiri, sugu) %>% tally() # andmed jäävad nimekirja järgi grupeerituks
valimised %>% count(nimekiri, sugu) # andmed ei jää grupeerituks
```


**Üsna hiljutine uuendus** on võimalus grupeerida andmeid ka `summarise()` funktsiooni sees, ilma `group_by()`-funktsiooni kasutamata. Sellisel juhul tuleb `summarise()`-funktsiooni sisse lisada argument `.by =`, mis täpsustab, mis tunnuse alusel andmeid rühmitatakse. Ka sellise kasutuse puhul ei jää andmed pärast funktsiooni kasutamist ühegi tunnuse alusel grupeerituks.     

```{r, eval = F}
valimised %>%
  summarise(keskmine_häältesaak = mean(hääli_kokku), .by = c(nimekiri, sugu))
```
...
kysimustik %>%
  ...
  summarise(vastajaid = n()) %>% # andmed jäävad aasta järgi grupeerituks
  mutate(prop = vastajaid/sum(vastajaid)) %>% # arvuta kohvi- ja teejoojate osakaalud iga aasta kohta ja lisa tulemus uude tulpa
  ungroup() %>%
  filter(lemmikjook == "Kohv") %>%
...
tapply(valimised$hääli_kokku, list(valimised$nimekiri, valimised$sugu), sum) %>%
  as.data.frame() %>%
  rownames_to_column(var = "nimekiri") %>%
...
Andmestikus `kiltsi_protokollid.xlsx` on ajaloolase Kersti Lusti koostatud andmestik Kiltsi vallakohtuprotokollide hagejate ja kostjate kohta aastatest 1872-1890. 
...
Praegu on andmestik laias formaadis, st iga protokolli kohta on üks rida ning selle osaliste info (ükskõik kui palju neid osalisi on) on laiali eraldi tulpades. See tähendab, et kui ühes protokollis võib olla tervelt 10 kostjat, on 10 kostja tulbad olemas kõikide protokollide jaoks, ehkki enamiku protokollide puhul on kohtuloos olnud ainult 1-2 kostjat korraga. Korrastame selle laias formaadis tabeli pikka formaati nii, et iga protokolli osaleja oleks eraldi real. Nii et kui protokollis on näiteks ainult üks hageja ja üks kostja, oleks selle protokolli kohta 2 rida; kui protokollis on üks hageja ja kaks kostjat, siis 3 rida jne.     
...
kiltsi <- readxl::read_excel("data/kiltsi_protokollid.xlsx", col_types = "text") # loeb tühjad lahtrid sisse NA-dena
```
...
kiltsi %>%
  pivot_longer(., # tee andmestikust pikk tabel
               cols = starts_with(c("H", "K"), ignore.case = F), # vali pika tabeli jaoks tunnused, mis algavad suure "H" või "K"-ga
...
Järgmiseks jätame andmestikust välja need read, kus tulbas `väärtused` on puuduvad väärtused (`NA`).  


```{r}
kiltsi %>%
  pivot_longer(.,
               cols = starts_with(c("H", "K"), ignore.case = F),
               names_sep = "_",
               names_to = c("nimi", "klass"),
               values_to = "väärtused") %>%
  filter(!is.na(väärtused)) # jäta välja read, kus tulbas "väärtused" on NA-d
```
...
kiltsi %>%
  ...
  mutate(roll = case_when(grepl("^H", nimi) ~ "hageja", 
                          grepl("^K", nimi) ~ "kostja")) %>%
...
Kas naisi hakkas meestega võrreldes aastate jooksul rohkem kohtus käima? Kas see erineb rolliti (hageja vs. kostja rollis)? Püüa vastata tulpdiagrammiga.   

Aasta on andmestikus vaikimisi sisse loetud tekstilise tunnusena.    

```{r, class.source = 'fold-hide'}
kiltsi_puhas %>%
  filter(!is.na(sugu)) %>%
  ggplot() +
  geom_bar(aes(fill = sugu, y = aasta), position = "fill")
...
Käsitleme aastat arvulise tunnusena ja kuvame ainult naiste osakaalusid.  
...
kiltsi_puhas %>%
  ...
  ungroup() %>%
  filter(sugu == "naine") %>%
  ggplot(aes(x = as.numeric(aasta), y = prop)) +
  geom_point() +
  geom_smooth() +
  ylim(c(0, 1))

kiltsi_puhas %>%
  group_by(aasta, roll, sugu) %>%
  summarise(n = n()) %>%
  mutate(prop =n/sum(n)) %>%
  ungroup() %>%
  filter(sugu == "naine") %>%
  ggplot(aes(x = as.numeric(aasta), y = prop, color = roll)) +
  geom_point() +
  geom_smooth() +
  ylim(c(0, 1))
```

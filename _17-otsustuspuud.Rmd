...
# Teeme puumudeli
puu1 <- ctree(cx ~ TR_liik + tegusõna_klass + LM_mobiilsus + LM_komplekssus + LM_pikkus_log + sõnajärg + murderühm, data = dat)
...
Miks siis see `TR_liik` ikkagi siin puus oluliseks osutus, kui ta uuritava tunnuse varieerumisega 1. sõlmes individuaalselt üldse seotud ei ole ning osutus ebaoluliseks ka regressioonis?  
...
puu4 <- ctree(haali_kokku ~ sugu + haridus + sodiaak, 
              data = kandidaadid, 
...
```{r}
puu5 <- ctree(log(haali_kokku) ~ sugu + haridus + sodiaak, 
              data = kandidaadid, 
              subset = haridus != "Algharidus")
plot(puu5)
```
...
puu6 <- ctree(log(haali_kokku) ~ sugu + haridus + sodiaak + kasutab_meili + 
                nime_pikkus + vanus, 
              data = kandidaadid, 
              subset = haridus != "Algharidus")
plot(puu6)
...
Kuna meil ei ole tegemist päris lineaarse seosega ja logaritmimine lähendab normaaljaotusele ainult tegelike reaktsiooniaegade jaotust, siis oleks hea kontrollida mudeli headuse hindamisel seost ka Spearmani korrelatsioonikordajaga.

```{r}
cor(enn, teg, method = "spearman")
```

Selleks, et puumudeli (ligikaudset) headust võiks võrrelda ka nt lineaarse regressioonimudeli omaga, võib korrelatsioonikordaja põhjal leida mudeli poolt seletatud varieerumise protsendi **R^2^**.
...
Teine variant jõuda sellesama Pearsoni-põhise R^2^-ni on lahutada 1st **koguruutude summa** (*Total Sum of Squares*) ja **jääkide ruutude summa** (*Residual Sum of Squares*) jagatis.
```{r}
...
Asendame näiteks esmalt oma 1444 vaatlusega andmestikus 300 juhuslikul vaatlusel staatilisuse liigutatavusega.
...
set.seed(1)
# Juhuslikult valitud ridade indeksid konstruktsiooni asendamiseks
mob <- sample(rownames(dat_mod[dat_mod$LM_mobiilsus == "staatiline",]), 300, replace = FALSE)

# Asenda 300 vaatlusel "staatiline" "liigutatav"-aga
dat_mod[mob,"LM_mobiilsus"] <- "liigutatav"

# Tee uus puu
puu_test <- ctree(cx ~ TR_liik + tegusõna_klass + LM_mobiilsus + LM_komplekssus + 
...
Nüüd saame puu, milles kõige tugevam seletav tunnus on hoopiski murderühm ja kohasõna liigutatavus saab oluliseks alles puu alumistes hargnemistes.  
...
Antud juhul sobib mudel enam-vähem kirjeldama ka testandmestikku, ent kui headuse näitajad test- ja treeningandmestiku vahel on väga erinevad, siis võib proovida mudelit erinevate parameetrite abil trimmida (vt `control = ctree_control()`).

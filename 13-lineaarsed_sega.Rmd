...
Kasutame eelmistest praktikumidest tuttavat välteandmestikku, mis on pärit artiklist Lippus, P., Asu, E. L., Teras, P., & Tuisk, T. (2013). Quantity-related variation of duration, pitch and vowel quality in spontaneous Estonian. Journal of Phonetics, 41(1), 17–28. https://doi.org/10.1016/j.wocn.2012.09.005

Selles töös uuriti eesti keele välte akustilisi tunnuseid *eesti keele spontaanse kõne foneetilisest korpuse* põhjal. Andmestikus on info 1714 kahesilbilise sõna kohta, mõõdetud on häälikute kestust, põhitooni liikumist, vokaalikvaliteeti jms. Samuti on märgitud sõnade välde, fookusrõhk jms.
...
Eelmistes praktikumides kasutasime seda andmestikku tavalise lineaarse mudeliga ja et täita mudeli eeldusi, et tegemist on sõltumatute mõõtmistega, keskmistasime andmeid nii, et iga kõneleja kohta oleks iga seletava tunnuse taseme kohta üks väärtus. (Umbes nii on ka Lippus jt 2013 artiklis analüüs läbi viidud.)

```{r, message=F}
dat2 %>% 
    filter(Struct == "cvcv") %>% 
    group_by(SP, Intonation, Quantity) %>% 
...
### Mitu juhuslikku vabaliiget ja kallet?

Ühes mudelis võib olla ka mitu juhuslikku faktorit, millest mõnel on ka juhuslikud kalded ja teisel ei ole. Juhuslik kalle on võimalik faktorite kombinatsioonis, kus ühe juhusliku faktori tasemega esinevad erinevad fikseeritud faktori tasemed. Näiteks siin vokaalikestuse ja välte mudelis saaksime lisada vältele juhusliku kõneleja kalde (sest üks kõneleja on öelnud sõnu erinevates väldetes), aga sõna saab ainult juhusliku lõikepunktina lisada (sest üks sõnavorm on alati samas vältes).

Siin näites saame ka hoiatuse *singulaarsuse* kohta, mis vihjab sellele, et mudel on liiga keerukas ja andmetes on varieerumine liiga väike.

Samuti peab arvestama, et iga juhusliku efekti lisamine võtab fikseeritud efektidelt seletusjõudu vähemaks (siin näites seda küll ei juhtunud).

```{r}
m4 <- lmer(V1 ~ Quantity + (1+Quantity|SP) + (1|Word), data = dat2, subset = Struct == "cvcv", REML=F)
anova(m3, m4)
summary(m4)
```


## Inkrementaalne modelleerimine segamudeliga

Nagu tavalise lineaarse mudeli puhul, tuleks segamudeliga lähtuda Occami printsiibist ja valida optimaalselt lihtne aga kõige tugevama seletusjõuga mudel. Mudeli headuse näitamiseks peaks lähtuma

- anova testiga mudeli jääkide võrdlemisest (võrreldes ühe faktori võrra erinevaid mudeleid)
- võrdlema Akaike informatsioonikriteeriumit (AIC)
- võrdlema pseudo-R-ruutu -- üldiselt segamudelitel pole R-ruutu (kuigi `mgcv` paketis GAM mudelil on). Sageli on selle asemel midagi R^2^ sarnast (*pseudo R^2^*), mis sarnaselet AIC-ile on number, mida sama andmestiku eri mudeliversioonide puhul saab võrrelda, aga mis ei ole tõlgendatav kui mudeli seletusjõu protsent.

Erinevad mudeli headuse näitajad võivad pisut erinevaid tulemusi anda optimaalse mudeli valikul.

```{r}
AIC(m1, m2, m3, m4)
```

AIC põhjal on parim *m4* (selle AIC väärtus on kõige väiksem).

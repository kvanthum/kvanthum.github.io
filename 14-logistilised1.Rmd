...
- Näiteks sellisele osalejale, kellel **ei ole kogemusi** kvantitatiivsete meetoditega ja kelle lemmikloom on glaucus atlanticus, ennustab mudel iga aastase kasvu kohta tema sünniaastas õpitud aastate arvu kahanemist `r abs(mod$coefficients[["synniaasta"]])` aasta võrra. Kui sellise osaleja sünniaasta oleks nt 1987, ennustaks mudel tema ülikoolis õpitud aastate arvuks `649.40718 + 1987 * (-0.32269)` ehk `r mod$coefficients['(Intercept)'] + 1987 * mod$coefficients['synniaasta']` aastat.  
- Sama sünniaastaga osalejale, kellel **on kogemusi** kvantitatiivsete meetoditega ja kelle lemmikloom on glaucus atlanticus, ennustab mudel õpitud aastate arvuks aga `649.40718 + 1987 * (-0.32269) + 1.08449` ehk `r mod$coefficients['(Intercept)'] + 1987 * mod$coefficients['synniaasta'] + mod$coefficients['kogemused_kvantJah']` aastat.  
- Sama sünniaastaga osalejale, kellel **on kogemusi** kvantitatiivsete meetoditega ja kelle lemmikloom on koer, ennustab mudel õpitud aastate arvuks `649.40718 + 1987 * (-0.32269) + 1.08449 -1.27974` ehk `r mod$coefficients['(Intercept)'] + 1987 * mod$coefficients['synniaasta'] + mod$coefficients['kogemused_kvantJah'] + mod$coefficients['lemmikloomKoer']`. 
...
Erinevuse **statistilist olulisust** näitavad koefitsientide *p*-väärtused. Kui koefitsiendi *p*-väärtus on > 0.05, ei ole statistiliselt olulist erinevust **võrreldes baaskontekstiga** (mis ei tähenda tingimata, et erinevus ei oleks oluline mingite teiste väärtuste vahel). Näiteks on ennustatud õpitud aastate arv oluliselt erinev osalejal, kelle hüpoteetiline sünniaasta on 0, ja osalejal, kelle hüpoteetiline sünniaasta on 1. Õpitud aastate arv aga ei erine oluliselt nende hulgas, kellel ei ole kvantitatiivsete meetoditega kogemusi (vabaliikme kontekst), ning nende hulgas, kellel on. Samuti ei õpitud aastate arv nende vastajate puhul, kelle lemmikloom on glaucus atlanticus (vabaliikme kontekst), nendest, kelle lemmikloom on kaelkirjak või koer, ent me ei tea seda, kas kaelkirjaku ja koera vahel võib olla statistiliselt olulisi erinevusi.  
...
- Ako Sauga (2020). [Statistika õpik majanduseriala üliõpilastele](https://digikogu.taltech.ee/et/Item/bdcc647f-fa18-47c1-b499-99024f8bcc93).  
- Sügis jt (2024). [Praktiline andmeteadus. Kõrgkooliõpik](https://courses.cs.ut.ee/t/andmeteadus/Main/HomePage?action=download&upname=Praktiline_andmeteadus_ko%CC%83rgkoolio%CC%83pik.pdf).  
...
**1. Vabaliikme `(Intercept)` koefitsient** `r m.glm$coefficients[[1]]` näitab *peal*-konstruktsiooni esinemise **šansi logaritmi / logaritmitud šanssi** (*logit*) tunnuse `LM_mobiilsus` baastaseme korral. Kategoriaalse **seletava tunnuse baastase on vaikimisi tähestikulises järjekorras kõige esimene väärtus** (NB! vastupidi sellele, kuidas valitakse uuritava tunnuse baastase) (`LM_mobiilsus` tunnuse puhul `liigutatav`, mitte `staatiline`). *Peal*-konstruktsiooni esinemise šansi logaritmi ennustatakse niisiis sellises kontekstis, kus kohasõna on mõtteliselt hõlpsasti liigutatav (nt *laud*). **Logaritmitud šansi saab teisendada tavaliseks šansiks astendamise funktsiooniga `exp()`, tõenäosuseks funktsiooniga `plogis()`**. Ehk kui logaritm alusel *e* (Euleri arv, umbes 2,71828) šansist on `r m.glm$coefficients[[1]]`, siis šanss on *e* astmes `r m.glm$coefficients[[1]]`. Vaikimisi kasutatakse niisiis naturaallogaritmi.    
...
Saame tõenäosuse leida ka funktsiooniga `plogis()`.  
...
# (enne astendamist liidame vabaliikmele koefitsiendi/koefitsientide väärtuse/väärtused)
...
Ka siin võime arvutada šansside asemel välja ka tõenäosused.  

```{r}
# "peal" kasutamise tõenäosus vabaliikme kontekstis (= kui LM_mobiilsus == "liigutatav")
plogis(m.glm$coefficients[["(Intercept)"]])

# "peal" kasutamise tõenäosus kontekstis, kus LM_mobiilsus == "staatiline" 
plogis(m.glm$coefficients[["(Intercept)"]] + m.glm$coefficients[["LM_mobiilsusstaatiline"]])
```

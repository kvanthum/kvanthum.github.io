...
Logistilise regressiooni mudel näeb välja sarnane lineaarse regressioonimudeli omaga. Funktsiooni `lm()` asemel kasutame funktsiooni `glm()`, kuna logistiline regressioon on üks mudel nn **üldistatud lineaarsete mudelite** (_**g**eneralized **l**inear **m**odels_) perest. Üldistatud lineaarseid mudeleid seob omavahel eeskätt see, et need ei eelda, et mudeli jäägid oleksid normaaljaotusega, ning nad võimaldavad luua erinevate funktsioonide abil justkui lineaarse seose tunnuste vahel, mille vahel tegelikult lineaarset seost ei ole (sellest natuke hiljem).  
...
m.jook <- glm(lemmikjook ~ lemmikloom + synniaasta, data = kysimustik, family = "binomial") 
...
Kogemuse omamise **tõenäosuse (*p*)** leidmiseks jagame kogemust omajate (= soodsate võimaluste) arvu kõikide vastajate arvuga ehk 44/(21+44).

```{r}
(p_Jah <- tab[["Jah"]]/sum(tab))
```

Kogemuse omamise tõenäosus andmestikus on ~0.68.  
Kogemuse mitteomamise tõenäosus on 1 - kogemuse omamise tõenäosus ehk 1-0.6769231=0.3230769.  
...
Kogemuse mitteomamise tõenäosus andmestikus on seega ~0.32.

Sündmuse toimumise (kogemuse omamise) ja sündmuse mittetoimumise (kogemuse mitteomamise) tõenäosused annavad liites kokku 1.
...
Kui kvantmeetodite kogemuse omamise tõenäosus on 0.68 ja mitteomamise tõenäosus on 0.32, siis **kogemuse omamise šanss** on 0.68/0.32 = ~ 2.125. 

Kogemust omatakse eeldatavasti niisiis **2.125 korda rohkem** kui ei omata.  

Kui teejoomise tõenäosus on 0.46 ja tee mittejoomise tõenäosus (= kohvijoomise tõenäosus) on 0.54, siis on **teejoomise šanss** 0.46/0.54 = 0.85.

```{r}
(P_Tee <- p_Tee/p_Kohv)
```

Teed juuakse **0.85 "korda rohkem"** kui ei jooda.  
Teisisõnu: teed juuakse vähem kui kohvi. 

Kui šanss oleks 1, siis on sündmuse toimumise ja mittetoimumise tõenäosused võrdsed (nt teed ja kohvi juuakse ühepalju).  
...
Prillidega üliõpilane B hilineb loengusse **~0,1 korda rohkem** kui jõuab õigeks ajaks? Mida see tähendab?
...
Kui šanss on väiksem kui 1, siis sündmuse toimumise tõenäosus on väiksem kui sündmuse mittetoimumise tõenäosus. 
Teisisõnu: kuna prillidega üliõpilase loengusse hilinemise šanss on 0.1, võime järeldada, et üliõpilase loengusse hilinemise tõenäosus on väiksem kui tõenäosus jõuda loengusse õigel ajal. 
...
Logistilises regressioonis hinnatakse seletava tunnuse mõju uuritava tunnuse ühe taseme esinemise tõenäosusele šansi logaritmi kaudu. Teisisõnu saaksime hinnata logistilise regressiooni mudeliga seda, kuidas üliõpilaste loengusse **hilinemise šansi logaritm** muutub vastavalt sellele, kas tegemist on prillikandjaga või mitte. 

Šansisuhet aga kasutame siis, kui tõlgendame logistilise regressiooni koefitsiente, mis näitavad, **mitu logaritmitud korda** šansisuhe seletava tunnuse muutumise korral muutub. 


```{r}

# Prillide kandmise mõju hilinemisele 1 loengu põhjal

df <- data.frame(nimi = c("Mari", "Jüri", "Ants", "Toomas", "Kertu",
                          "Peeter", "Sille", "Marianne", "Juhan", "Oskar",
                          "Tuuli", "Reelika", "Annika", "Rauno", "Joosep",
...
- prillideta üliõpilaste grupis on tõenäosus loengusse hilineda 0,7 ja tõenäosus mitte hilineda 0,3 ning šanss hilineda seega 0,7/0,3=2,33333,    
- prillidega üliõpilaste grupis on tõenäosus loengusse hilineda 0,1 ja tõenäosus mitte hilineda 0,9 ning šanss hilineda seega 0,1/0,9=0,11111,  
...
- näitab mudeli vabaliikme väärtus hilinemise **šansi logaritmi** prillideta üliõpilaste seas,  
- koefitsient šansi muutust logaritmitud kordades (ehk **logaritmitud šansisuhet**), kui vaatame prillikandjate gruppi: mitu (logaritmitud) korda muutub šanss loengusse hilineda, kui muudame seletava tunnuse taset?    

```{r}
exp(logmod$coefficients[1]) # prillideta üliõpilaste loengusse hilinemise šanss
exp(logmod$coefficients[2]) # prillideta üliõpilaste loengusse hilinemise šansi ja prillidega üliõpilaste loengusse hilinemise šansi suhe/jagatis (mitu korda "rohkem" hilinevad prillidega üliõpilased?)
exp(logmod$coefficients[1] + logmod$coefficients[2])# prillidega üliõpilaste loengusse hilinemise šanss
```
...
Siin räägime põhiliselt binomiaalsest logistilisest regressioonist, kus on kahe tasemega uuritav tunnus. See on aluseks ka multinomiaalsele regressioonile.  
...
```{r, echo = F}
arvud <- c(1, 4, 8, 15, 30, 70, 100, 200, 500, 900, 2000, 10000)
...
```{r}
# Kaassõna esinemise tõenäosus
(p_peal <- tab[["peal"]]/sum(tab)) # NB! väike p-täht

# Kaassõna esinemise šanss
(P_peal <- p_peal/(1-p_peal)) # NB! suur p-täht
```
```{r}
# Kaassõna mitte-esinemise tõenäosus
(p_ade <- tab[["ade"]]/sum(tab))

# Kaassõna mitte-esinemise šanss
(P_ade <- p_ade/(1-p_ade))
```
...
Ehkki *peal*-konstruktsioon tundub olevat nii liigutatavate kui ka staatiliste "kohtadega" (orientiiridega) sagedam, on staatiliste kohtade puhul kaassõna kasutus oluliselt vähem tõenäoline (nt *heinamaa peal*) kui liigutatavate kohtadega (nt *laua peal*).  



### Logistilise regressiooni mudel

Teeme nüüd lihtsal logistilise regressiooni mudeli, et kontrollida, kas kohakonstruktsiooni valikut saab seletada koha/orientiiri liigutatavuse kaudu. Tegelikkuses on mudeli kasutamine kasulikum muidugi pigem siis, kui vaadata mitme seletava tunnuse mõju uuritava tunnuse varieerumisele.

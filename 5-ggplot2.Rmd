...
load("data/kysimustik_2025.RData")
library(ggplot2)
...
# Laadi andmestik
load("kysimustik_2025.RData")

# Koosta graafik
ggplot(kysimustik, aes(x = synniaasta, y = kaua_opid, color = kogemused_kvant)) +
  geom_point(alpha = 0.7) +  # Hajuvusdiagramm
  facet_wrap(~ lemmikjook) + # Tee- ja kohvijoojate eraldi paneelid
  scale_color_manual(values = c("Jah" = "orange", "Ei" = "grey45")) +  # Värvid kategooriatele
  labs(
    title = "Õpitud aastate arv vastavalt sünniaastale",
    subtitle = "ja kokkupuutele kvantitatiivsete meetoditega",
    x = "Sünniaasta",
    y = "Õpitud aastate arv",
    color = "Kogemused kvantitatiivsete meetoditega"
  ) +
  theme_minimal(base_size = 14) +  # Minimaalne teema
  theme(
    plot.background = element_rect(fill = "aliceblue", color = NA),  # Taustavärv

...
Ggploti spikri leiad [GitHubist](https://github.com/rstudio/cheatsheets/blob/master/data-visualization.pdf), levinumad funktsioonid ja rakendused lehelt [R Statistics](https://r-statistics.co/Top50-Ggplot2-Visualizations-MasterList-R-Code.html) või [R Charts](https://r-charts.com/ggplot2/).  
...
3. kandidaatide **vanuse** jaotumist vastavalt **soole** (kas meeskandidaadid olid vanemad kui naiskandidaadid?)  
...
**`geom_bar()`** on funktsioon, mis loeb kategoriaalse tunnuse tasemed vaikimisi kokku ja paneb y-teljele tasemete sagedused, ilma et peaksime sageduste leidmiseks esmalt nt `table()`-funktsiooni kasutama ja seda `data.frame`'iks teisendama (vrd nt baasgraafika `barplot()` funktsiooniga, mis nõuab sisendiks sagedustega `table`-objekti).  
...
Sektordiagrammide jaoks on aga parem ggploti täiendav pakett [`ggtricks`](https://cran.r-project.org/web/packages/ggtricks/readme/README.html), milles on funktsioon `geom_pie()`.  
...
Sellisel juhul ei eeldata, et iga lisatud kiht kasutab täpselt sama andmestikku ja samu tunnuseid, vaid iga kiht võib kasutada **erinevaid andmestikke** või **sama andmestiku erinevaid alamhulki**.  

Näiteks võime samal graafikul kujutada lisaks eraldi **joonega** ainult mingi valimisnimekirja, nt sotsiaaldemokraatide mees- ja naiskandidaatide arvu. **`geom_line`** on nn kollektiivne *geom* ehk selle joonistamiseks kasutatakse mitme vaatluse andmeid (joont ei saa joonistada ainult 1 vaatluse põhjal). Selleks, et joon oleks tõmmatud üle kõikide kategooriate (siin mees- ja naiskandidaatide tulpade), kasutame `geom_line()` funktsiooni sees argumenti `group`.  
...
ggplot() +
  ...
  geom_line(data = valimised[valimised$nimekiri == "Sotsiaaldemokraatlik Erakond",],
            ...
            stat = "count",
            color = "tomato1",
            group = 1) # vaatame küll ainult sotside nimekirja kandidaate, aga neid omakorda edasi ei grupeeri ükski andmestiku tunnus (group = konstant)

```


Kui aga tahta grupeerida vaatlusi mingi andmestiku tunnuse järgi (nt valimisnimekiri) ning näidata seega **kõiki** selle tunnuse alusel tekkivaid andmestiku alamosi korraga, tuleb `group`-argument liigutada `aes()` funktsiooni sisse ning argumendi väärtuseks panna seesama andmestiku tunnus ise.   
Sel juhul ei tule iga tunnuse taseme kohta tekitada uut `geom`-kihti omaette alamandmestikuga (nt `valimised[valimised$nimekiri == "Sotsiaaldemokraatlik Erakond",]`, `valimised[valimised$nimekiri == "Eesti Reformierakond",]` jne). Kuvame näiteks kõikide erakondade mees- ja naiskandidaatide arvu üldiste tulpade peal eri värvi joontena.
...
<span style="color: black;">Lisa graafikule pealkiri "Õpitud aastate arv vastavalt sünniaastale", alapealkiri "ja kokkupuutele kvantitatiivsete meetoditega" ning legendi pealkiri "Kogemused kvantitatiivsete meetoditega".</span>  
...
Lisa graafikule pealkiri "Õpitud aastate arv vastavalt sünniaastale", alapealkiri "ja kokkupuutele kvantitatiivsete meetoditega", sobivad x- ja y-telje pealkirjad ning legendi pealkiri "Kogemused kvantitatiivsete meetoditega".</span>  
...
Kuva sünniaasta ja õpitud aastate arvu suhet hajuvusdiagrammil.  
Tähista erineva värviga need vastajad, kellel on kvantitatiivsete meetoditega kogemust, ja need, kellel ei ole. Kasuta kategooriate jaoks värve "grey45" ja "orange".  
Lisa graafikule pealkiri "Õpitud aastate arv vastavalt sünniaastale", alapealkiri "ja kokkupuutele kvantitatiivsete meetoditega", sobivad x- ja y-telje pealkirjad ning legendi pealkiri "Kogemused kvantitatiivsete meetoditega".   
...
![](imgs/ggplot_chatgpt.png){width=70%}
...
ggplot(kysimustik,
  ...
  geom_point() + # Hajuvusdiagramm
  facet_wrap(~ lemmikjook) + # Tee- ja kohvijoojate eraldi paneelid
  scale_color_manual(
    values = c("grey45", "orange"), # Värvid kategooriatele
    name = "Kogemused kvantitatiivsete meetoditega"
  ) +
  labs(
    title = "Õpitud aastate arv vastavalt sünniaastale",
    subtitle = "ja kokkupuutele kvantitatiivsete meetoditega",
    x = "Sünniaasta",
    y = "Õpitud aastate arv"
  ) +
  theme_minimal() + # Minimaalne teema
  theme(
    plot.background = element_rect(fill = "aliceblue", colour = NA), # Joonise taustavärv
    panel.background = element_rect(fill = "aliceblue", colour = NA), # Paneelide taustavärv
    plot.title = element_text(hjust = 0.5), # Pealkiri keskele
    plot.subtitle = element_text(hjust = 0.5) # Alapealkiri keskele
  )
...
ggplot(kysimustik, 
       aes(y = kaua_opid, 
           x = seq_along(kaua_opid))) + # vektor, mis koosneb arvudest 1...151
...
ggplot(kysimustik, 
  ...
  scale_y_continuous(breaks = seq(0, 100, 5)) + # y-teljel väärtused 0st 100ni viiese sammuga
...
Kumbki graafikapakett ei ole olemuslikult parem kui teine ja on suur tõenäosus, et joonist, mida saad teha ggplotis, saad teha ka baasgraafikas (või hoopis mingi muu graafikapaketiga, nt `lattice`). Kummaga täpselt töötada, sõltub isiklikest eelistustest, harjumusest ja veidi ka sellest, milliseid graafikuid ja mille jaoks tüüpiliselt kasutatakse. Baasgraafikapaketiga saab enamasti kiiremini oma andmetest ülevaate, ggplot pakub veidi enam võimalusi nt rakendustes või veebilehtedel jooksvateks joonisteks ning on laialtlevinud andmeteaduses. Publitseerimiseks tuleb mõlema paketiga töötades joonistega pisut vaeva näha. Mõlemal paketil on ka terve hulk laiendusi, mis võimaldavad teha asju, millega pakett ise toime ei tule (nt 3D graafikud ggplotis). Asjad, milles ggplotil on siiski selge eelis, on nt legendid, grupeerimine ja paneelid (*facets*). Loe ka [siit](http://varianceexplained.org/r/why-I-use-ggplot2/).
...
<br>
Sobiva joonisetüübi valimisel on kindlasti abiks aga leht <https://www.data-to-viz.com/> ja juba mainitud <https://r-charts.com/>.  


## Interaktiivsed graafikud ja plotly

Paketis `plotly` on funktsioon `ggplotly()`, mis võimaldab teha interaktiivseid graafikuid.  
...
```{r, warning=FALSE, message=FALSE}
# Teeme esmalt tavalise ggploti graafiku ja salvestame selle mingisse objekti, nt nimega 'graafik1'
ggplot() +
  geom_bar(data = valimised, aes(x = sugu)) +
  geom_line(data = valimised, aes(x = sugu, group = nimekiri, color = nimekiri),
            stat = "count") -> graafik1

# Paneme tehtud graafikuobjekti funktsiooni ggplotly() sisse
# toolip-argumendis saame täpsustada, milliseid graafiku elemente hiirega joonisel liikudes näeb
ggplotly(graafik1, tooltip = c("y", "colour"))
```


```{r, warning=FALSE, message=FALSE}
ggplot(kysimustik, aes(x = synniaasta, y = kaua_opid, color = kogemused_kvant)) +
  geom_point() +
  scale_color_manual(values = c("grey45", "orange")) +
  labs(title = "Õpitud aastate arv vastavalt sünniaastale", subtitle = "ja kokkupuutele kvantitatiivsete meetoditega", x = "Sünniaasta", y = "Õpitud aastate arv", color = "Kogemused\nkvantitatiivsete\nmeetoditega") +
  theme(plot.title = element_text(hjust = 0.5),
        plot.subtitle = element_text(hjust = 0.5),
        panel.background = element_rect(fill = "aliceblue")) -> graafik2

ggplotly(graafik2, tooltip = "all")
```
...
Hex-värve saab kombineerida värvipaletiks lugematul hulgal veebilehtedel, nt lehel [Coolors](https://coolors.co/). 
Oma värvipaletti saab seejärel R-is mingil joonisel kasutada.

```{r}
# Erinevad viisid värvide täpsustamiseks
minupalett <- c(rgb(147,78,78,maxColorValue = 255), # punakaspruun
                "#7faf90", # tuhmroheline
                "navyblue", # tumesinine
                "#8e7e24") # tumekollane

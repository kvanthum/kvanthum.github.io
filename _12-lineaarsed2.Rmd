...
Kuna siin on 0-sagedusega sõnu ja
```{r}
log(0)
```
...
- Kuna *sagedus* on mudelis logaritmitud, siis seda peame ka mudeli prognooside arvutamisel arvestama. Selle kordajaga peaks korutama logaritmitud sagedusi.
...
plot(Mean_RT ~ Length, data = ldt1, 
     ...
     main = "Sõna pikkuse mõju keskmise sagedusega sõnadel")
lines(x = 3:14, 
...
plot(Mean_RT ~ log1p(Freq), data = ldt1, 
     ...
     main = "Sõna sageduse mõju keskmise pikkusega sõnadel", 
     axes=F)
lines(x = 0:12, 
      y = (ldt1.mod1$coefficients["(Intercept)"] + 
               mean(ldt1$Length) * ldt1.mod1$coefficients["Length"] + 
...
- Tärniga saab tähistada lühidalt faktorite vahelist peamõju ja interakstiooni korraga, tulemus on sama, mis eelmisel real: y ~ x1 * x2
...
Kui enne vaatasime ainult peamõjudega sõna pikkuse ja sageduse efekti reaktsiooniajale, siis nüüd lisame ka interaktsiooni. R-i `lm()` süntaksis võiks selleks lihtsalt plussi (+) tärniga (*), aga kirjutame siin praegu pikalt välja:
...
- Võib testida tunnuste vahelist korrelatsiooni.

```{r}
cov2cor(cov(kandidaadid[,c("e_haaled", "haaled_valisriigist", "kokku_ringkonnas","nr")]))
```
...
```{r, eval=T}
table(kandidaadid$erakondlik_kuuluvus) # üks ühene rühm
table(kandidaadid$nimekiri_voi_uksikkandidaat)
```


```{r, eval=F}
table(kandidaadid$haridus) # eemalda ühene rühm
table(kandidaadid$sodiaak)
```

Kandidaatide töökohtadest oleks vist liiga keeruline mingit selgete piiridega faktorit kujundada ilma käsitsi sekkumata :(
```{r, eval=F}
head(kandidaadid$tookoht_ja_amet) 
```
...
```{r, eval=T}
head(kandidaadid$kontaktandmed)
length(grep("@", kandidaadid$kontaktandmed))
```
...
```{r, eval=F}
head(kandidaadid$nimi) 
summary(nchar(kandidaadid$nimi))
```
```{r, eval=T}
...
```{r, eval=T}
is(kandidaadid$sunniaeg)
head(kandidaadid$sunniaeg)
...
```{r}
m0 <- lm(log(haali_kokku) ~ 1, data = kandidaadid, subset = haridus != "Algharidus")
summary(m0)
```
...
```{r}
m1 <- lm(log(haali_kokku) ~ haridus, data = kandidaadid, subset = haridus != "Algharidus")
anova(m0, m1)
```
...
## Aikaike informatsioonikriteerium
...
Käsk `step()` kasutab Aikaike informatsioonikriteeriumit (AIC), et ehitada samm-sammuliselt faktoreid lisades või eemaldades optimaalne mudel. Proovime leida faktoreid lisades parima mudeli valimistulemuste seletamiseks.
...
```{r, eval=T}
m_step <- step(lm(log(haali_kokku) ~ 1, data = kandidaadid, subset = haridus != "Algharidus"), scope = log(haali_kokku) ~ nimek * ringkond * haridus * sugu * vanus * sodiaak * kasutab_meili * nime_pikkus, direction = "forward")
```
...
```{r, echo=T}
TukeyHSD(aov(log(haali_kokku) ~ haridus + sugu + haridus:sugu, data = kandidaadid, subset = haridus != "Algharidus"))
```

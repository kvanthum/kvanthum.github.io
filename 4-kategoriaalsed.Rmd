...
Eelmine kord tegelesime arvuliste andmete kirjeldamise ja visualiseerimisega ning kasutasime andmestikku failis `kysimustik_2025.RData`. 
```{r}
...
Kasutame edasi andmestikku `kysimustik_2025.RData`.  
...
Praegusel juhul saabki maksimaalne variatsioonisuhe olla 0.75, sest kui jagada 126 vaatlust võrdselt 4 kategooriasse (= maksimaalne andmete hajuvus), on meil 4 moodi (igaühe sagedus 31.5, ehkki päriselt peaks sagedus olema täisarv) ja `1-(31.5/126) = 0.75`. Kohe, kui mõnda kategooriasse mõne teise arvelt vaatlusi juurde lisada (nt 31, 31, **33**, 31), muutub see moodiks ja variatsioonisuhe väheneb: `1-(33/126) = 0.74`.    

```{r, echo = F, fig.width=15, fig.height=7, warning=FALSE, message=FALSE}
library(tidyverse)
c(31.5,31.5,31.5,31.5) -> cat4
c(31,31,33,31) -> cat4_unb
c(31,31,59,5) -> cat4_unb2
c(1,1,123,1) -> cat4_unb3
...
Kui meil oleks ainult 2 kategooriat (nt koera vastuseid 63 ja kaelkirjaku vastuseid 63), siis saaks maksimaalne variatsioonisuhe olla `1-(63/126) = 0.5`. Kui meil oleks 14 kategooriat ja igas kategoorias võrdselt vastuseid (9), saaks maksimaalne variatsioonisuhe olla aga tervelt `1-(9/126) = 0.93`. 
...
data.frame(Lemmikloom = c(levels(kysimustik$lemmikloom), "Kss", "Lhm", "Kru", "Jns", "Pdr", "Hnt", "Rbn", "Ilvs", "Sga", "Lms"), Sagedus = cat14) %>% 
  ggplot(aes(x = Lemmikloom, y = Sagedus)) + 
  geom_col(fill = rep("red", 14)) +
  geom_text(aes(label = Sagedus), vjust = -1, size = 5) +
  labs(title = paste0("Variatsioonisuhe ", round(1-max(cat14)/sum(cat14), 2))) +
  scale_y_continuous(limits = c(0, 126)) +
  theme_void() + 
  theme(axis.text.x = element_text(size = 14), plot.title = element_text(hjust = 0.5),
        plot.background = element_rect(color = "grey80")) -> p_cat14

gridExtra::grid.arrange(p_cat2, p_cat14, ncol = 2)
```
...
p_cat14 + labs(title = paste0("Hälve moodist ", round(14/(14-1)*(1-max(cat14)/sum(cat14)), 2))) -> p2_cat14
...
c(rep(1, 13), 113) -> cat14_unb
data.frame(Lemmikloom = c(levels(kysimustik$lemmikloom), "Kss", "Lhm", "Kru", "Jns", "Pdr", "Hnt", "Rbn", "Ilvs", "Sga", "Lms"), Sagedus = cat14_unb) %>% 
  ggplot(aes(x = Lemmikloom, y = Sagedus)) + 
  geom_col(fill = c(rep("grey80", 13), "red")) +
  geom_text(aes(label = Sagedus), vjust = -1, size = 5) +
  labs(title = paste0("Hälve moodist ", round(14/(14-1)*(1-max(cat14_unb)/sum(cat14_unb)), 2))) +
  scale_y_continuous(limits = c(0, 126)) +
  theme_void() + 
  theme(axis.text.x = element_text(size = 14), plot.title = element_text(hjust = 0.5),
        plot.background = element_rect(color = "grey80")) -> p2_cat14
...
```{r, echo = F, fig.width=15, fig.height=7, warning=FALSE, message=FALSE}
grid.arrange(p2_cat2, p2_cat14, ncol = 2)
```

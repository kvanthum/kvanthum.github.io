...
Eelmine kord tegelesime arvuliste andmete kirjeldamise ja visualiseerimisega ning kasutasime andmestikku failis `kysimustik_2026.RData`. 
```{r}
...
- Keskmist, standardhälvet ja mediaani erinevates **gruppides** saab arvutada näiteks käsuga `tapply()`: `tapply(kysimustik$kaua_opid, kysimustik$lemmikjook, mean)`, `tapply(kysimustik$kaua_opid, list(kysimustik$lemmikjook, kysimustik$lemmikloom), mean)`
...
Kui oled praeguseks leidnud mõne andmestiku, millega oleks huvitav töötada, ning selles andmestikus on ka arvulisi tunnuseid, loe andmestik R-i. Kui sul veel sellist andmestikku ei ole, siis kasuta [1. praktikumi 3. ülesandes](https://kvanthum.github.io/1-sissejuhatus.html#%C3%BClesanne-3) kirjeldatud andmestikku `Pilot_Est_Aus.csv` või uuri R-i `datasets` paketi pakutavaid lihtsaid andmestikke.  

```{r, eval = F}
library(help = "datasets") # üldine loend andmestikest

?InsectSprays # mingi konkreetse andmestiku kirjeldus
?warpbreaks
?Orange
?penguins

data(penguins) # laadi valitud andmestik R-i
```


1. Leia mõne arvulise tunnuse keskväärtus ja standardhälve. Kui homogeensed on väärtused?    
2. Tee arvulise tunnuse jaotumisest karpdiagramm ja histogramm. Kas tunnuse jaotus on sümmeetriline?  
3. Leia sama arvulise tunnuse keskväärtus ja standardhälve mingi kategoriaalse tunnuse väärtuste rühmades. Kas rühmad on erinevad või sarnased?    
4. Tee karpdiagramm sama arvulise tunnuse jaotumisest kategoriaalse tunnuse rühmades. Kas rühmad on erinevad või sarnased?    
5. Tee hajuvusdiagramm, mille ühel teljel on valitud arvuline tunnus ning teisel teljel mõni teine arvuline tunnus. Kas muutus ühes arvulises tunnuses tundub toovat kaasa muutuse ka teises tunnuses?    
...
Kasutame edasi andmestikku `kysimustik_2026.RData`.  
...
# Lühendada võib ka nt funktsiooniga abbreviate()
loomavec <- c("Glaucus atlanticus", "Kaelkirjak", "Koer", "Mulle ei meeldi loomad")
abbreviate(loomavec)
```
...
Vaikimisi ei näidata sagedustabelites puuduvaid väärtusi (*NA*). Kui tahame ka neid näha, saame seda funktsioonis `table()` täpsustada.  

```{r, results='hide'}
table(kysimustik$lemmikloom, useNA = "ifany") # näita NA-sid ainult siis, kui neid esineb
table(kysimustik$lemmikloom, useNA = "always") # näita NA-sid alati
```
...
```{r}
# sulud kõige ümber: suuna tabel objekti t.jook.loom JA trüki tabel konsooli
...
aggregate(kysimustik$synniaasta, list(kysimustik$lemmikjook), median) # rühmitav tunnus peab olema alati listina!
...
names(which.max(t.loom)) # mood
names(which.min(t.loom)) # lihtsalt kõige harvemini esinev väärtus
```

`which.max()` annab meile niisiis tabeli kõige sagedasema elemendi *indeksi* ehk järjekorranumbri. `names()` funktsiooni abil saame küsida ainult moodi ehk elementi ennast (st tabeli tulbanime) ilma sageduseta.  
...
- R-i pakett `qualvar`: [https://joelgombin.github.io/qualvar/index.html](https://joelgombin.github.io/qualvar/index.html)  
...
Variatsioonisuhe ei peegelda aga seda, kui palju neid teisi vastusevariante on, nt kas need 48% vastustest lähevad jagamisele kahe, kolme, nelja, kümne või kasvõi saja vastusevariandi vahel. 

Praegusel juhul saabki maksimaalne variatsioonisuhe olla 0.75, sest kui jagada 151 vaatlust võrdselt 4 kategooriasse (= maksimaalne andmete hajuvus), on meil 4 moodi (igaühe sagedus 37.5, ehkki päriselt peaks sagedus olema täisarv) ja `1-(37.5/151) = 0.75`. Kohe, kui mõnda kategooriasse mõne teise arvelt vaatlusi juurde lisada (nt 37, 37, **40**, 37), muutub see moodiks ja variatsioonisuhe väheneb: `1-(40/151) = 0.74`.    

```{r, echo = F, fig.width=15, fig.height=7, warning=FALSE, message=FALSE}
library(tidyverse)
c(37.5,37.5,37.5,37.5) -> cat4
c(37,37,40,37) -> cat4_unb
c(37,37,72,5) -> cat4_unb2
c(1,1,148,1) -> cat4_unb3
...
Kui meil oleks ainult 2 kategooriat (nt koera vastuseid 75 ja kaelkirjaku vastuseid 75), siis saaks maksimaalne variatsioonisuhe olla `1-(75/150) = 0.5`. Kui meil oleks 15 kategooriat ja igas kategoorias võrdselt vastuseid (10), saaks maksimaalne variatsioonisuhe olla aga tervelt `1-(10/150) = 0.93`. 
...
data.frame(Lemmikloom = c(levels(kysimustik$lemmikloom), "Kss", "Lhm", "Kru", "Jns", "Pdr", "Hnt", "Rbn", "Ilvs", "Sga", "Lms", "Hms"), Sagedus = cat15) %>% 
  ggplot(aes(x = Lemmikloom, y = Sagedus)) + 
  geom_col(fill = rep("red", 15)) +
  geom_text(aes(label = Sagedus), vjust = -1, size = 5) +
  labs(title = paste0("Variatsioonisuhe ", round(1-max(cat15)/sum(cat15), 2))) +
  scale_y_continuous(limits = c(0, 151)) +
  theme_void() + 
  theme(axis.text.x = element_text(size = 14), plot.title = element_text(hjust = 0.5),
        plot.background = element_rect(color = "grey80")) -> p_cat15

gridExtra::grid.arrange(p_cat2, p_cat15, ncol = 2)
```
...
p_cat15 + labs(title = paste0("Hälve moodist ", round(14/(14-1)*(1-max(cat15)/sum(cat15)), 2))) -> p2_cat15

gridExtra::grid.arrange(p2_cat4, p2_cat4_unb, p2_cat4_unb2, p2_cat4_unb3, ncol = 4)
```
...
c(rep(1, 14), 137) -> cat15_unb
data.frame(Lemmikloom = c(levels(kysimustik$lemmikloom), "Kss", "Lhm", "Kru", "Jns", "Pdr", "Hnt", "Rbn", "Ilvs", "Sga", "Lms", "Hms"), Sagedus = cat15_unb) %>% 
  ggplot(aes(x = Lemmikloom, y = Sagedus)) + 
  geom_col(fill = c(rep("grey80", 14), "red")) +
  geom_text(aes(label = Sagedus), vjust = -1, size = 5) +
  labs(title = paste0("Hälve moodist ", round(15/(15-1)*(1-max(cat15_unb)/sum(cat15_unb)), 2))) +
  scale_y_continuous(limits = c(0, 151)) +
  theme_void() + 
  theme(axis.text.x = element_text(size = 14), plot.title = element_text(hjust = 0.5),
        plot.background = element_rect(color = "grey80")) -> p2_cat15


gridExtra::grid.arrange(p2_cat2, p2_cat15, ncol = 2)
```
...
```{r, eval = FALSE}
barplot(kysimustik$kogemused_kvant)
```

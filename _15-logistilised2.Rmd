...
# põhimõtteliselt võib see olla ka x, y, tralllallaa või midagi muud
...
dat %>%
  select(cx, seletavad_kat) %>% # vali uuritava ja kat. seletavate tunnuste tulbad
  pivot_longer(., 2:length(.), # tee seletavate tunnuse põhjal laiast tabelist pikk
               names_to = "tunnused", # kus tunnuste nimed lähevad ühte tulpa
               values_to = "väärtused") %>% # ja nende tasemed/väärtused teise
  ggplot(aes(x = väärtused, # pane tulpdiagrammi x-teljele tunnuste väärtused
  ...
  stat_count(geom = "text", # lisa tekstina ka absoluutarvud
             aes(label = stat(count)), # tekstisildi jaoks loe kategooriate sagedused kokku
  ...
  coord_flip() # keera joonis külili
...
Andmetest kiire esmase ülevaate saamiseks on R-is ka erinevaid pakette, nt [`DataExplorer`](https://cran.r-project.org/web/packages/DataExplorer/vignettes/dataexplorer-intro.html), [`GGally`](https://ggobi.github.io/ggally/index.html) või [`Hmisc`](https://hbiostat.org/R/Hmisc/), mis hõlbustavad oluliselt andmeanalüütiku igapäevatööd.    

Samuti saab mingit esmast kirjeldavat analüüsi teha paketiga [`sjPlot`](https://strengejacke.github.io/sjPlot/reference/index.html), mida kasutame hiljem ka mudeli efektide visualiseerimiseks.  
...
Kuna mudeli vabaliikme koefitsiendi p-väärtus on väike, peame nullhüpoteesi hülgama ning tõdema, et juhul, kui me ei tea mitte midagi kohakonstruktsioonide esinemiskonteksti kohta, ei ole *peal*- konstruktsiooni ja alalütleva käände konstruktsiooni kasutuse tõenäosused võrdsed.  

## Mitme seletava tunnusega mudel

Lisame nüüd mudelisse veel tunnuseid.

Nagu varemgi räägitud, on mudelite tegemise puhul kaks põhilist lähenemist:  
...
Konfirmatoorse lähenemise puhul võime kõik oma andmestiku tunnused panna korraga mudelisse ning raporteerida lihtsalt, millised neist olid olulised, millised mitte, ja mis suunalised oluliste tunnuste mõjud olid.
...
Seepärast on konfirmatoorses analüüsis parem kasutada **II tüüpi ANOVAt**, mis võrdleb iga tunnuse mõju koos kõikide teiste lisatavate tunnustega. Selleks on funktsioon `Anova()` paketist `car` või funktsioon `drop1`.    
...
Tulba `LRT` ehk *likelihood-ratio test* põhjal saab nüüd otsustada, et kõige olulisemad tunnused *peal*-konstruktsiooni ja käändekonstruktsiooni valikul on kohasõna liigutatavus, murderühm, kohasõna komplekssus ning tegusõna klass. 
...
Vaatame siin paketti `sjPlot`, mis on loodud sotsiaalteaduste statistiliste analüüside visualiseerimiseks.  
...
Kui mudelis on mingite tunnuste vahel oluline interaktsioon, ei saa mudeli peamõjusid enam üksinda tõlgendada. Nii tuleks pikkuse mõju tõlgendada mudelis `m5.glm` ainult koos murde mõjuga. Kesk-murderühmas pikkusel niisiis mõju ei ole, Lõuna-murderühmas aga kasvab *peale*-konstruktsiooni kasutamise tõenäosus oluliselt iga sõnapikkuse logaritmitud ühiku (ehk iga arvu 2 astme) kohta umbes `exp(-0.14+0.61)=1.599994` korda, Saarte-murderühmas  `exp(-0.14+0.65)=1.665291` korda.  

```{r}
plot_model(m5.glm, type = "int")
```

Murrete mõju saame tabelis eraldi hinnata jällegi ainult juhul, kui kohasõna pikkuse logaritm on 0, st 1-silbiliste sõnade puhul. Kui sõnad lähevad pikemaks, peame kohe arvestama ka koosmõjuga.  

```{r}
plot_model(m5.glm, type = "pred", terms = c("murderühm", "LM_pikkus_log"))
```
...
#### 4. Tee kursuse küsimustiku andmete pealt mudel, mis ennustab seda, kas vastajal on kogemusi kvantitatiivsete meetoditega või mitte, vastavalt küsimustikule vastamise aasta ja ülikoolis õpitud aastate arvu järgi. Milline väide on õige?
```{r}
m1 <- glm(kogemused_kvant ~ aasta + kaua_opid, data = kysimustik, family = "binomial")
```

a) Mõlemad seletavad tunnused teevad mudelit oluliselt paremaks.
b) Ainult vastamise aasta tuleks mudelisse alles jätta.
c) Ainult õpitud aastad tuleks mudelisse alles jätta.
d) Tuleks lisada ka aasta ja õpitud aastate interaktsioon.

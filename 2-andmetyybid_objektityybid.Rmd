...
## Tunnuste jagunemine {.tabset .tabset-pills}
...
- __Kvalitatiivsed__/mittearvulised/kategoriaalsed:  
    * __nominaalskaalal__: kaks või enam kategooriat, mis teineteist välistavad. Peetakse kõige ebatäpsemaks ja vähem informatiivseks tunnusetüübiks. Samas on just nominaalskaala tunnused humanitaar- ja sotsiaalteaduste andmestikes väga levinud. <br> Nt sugu (_mees_/_naine_/_muu_), tegusõna sihilisus (_sihiline_/_sihitu_), kääne (_nimetav_/_omastav_/_osastav_), _nimi_, _kogumispunkt_, _teema_ jne.  
...
- __Kvantitatiivsed__/arvulised:
    * __intervalli- e vahemikskaalal__: esitab võrdseid erinevusi skaala punktide vahel. <br> Nt _temperatuur (Celsiuse või Fahrenheiti skaalal!)_.
...
kategs <- function(tunnus, pealkiri, aspekt, loomad, ord, caption = ""){
    ...
    if(loomad){
        p1 +
            annotation_custom(rasterGrob(koer), xmin = 3.5, xmax = 4.5, ymin = 0.5, ymax = 7.5) +
            annotation_custom(rasterGrob(kaelkirjak), xmin = 2.5, xmax = 3.5, ymin = 0.5, ymax = 7.5) +
            annotation_custom(rasterGrob(glaucus), xmin = 1.5, xmax = 2.5, ymin = 0.5, ymax = 7.5)
    }else{
        p1
    }
}
...
nums <- function(tunnus, tyyp, pealkiri){
  ...
  if(tyyp == "tulp"){
    ...
    p +
      ...
      theme(axis.text.y = element_blank(), axis.line.y = element_line(color = "grey80")) +
...
library(gridExtra)
grid.arrange(p8, grid.arrange(p9, p10, p11, nrow = 1), ncol = 1, heights = c(0.7,0.3))
```
...
Arvud võivad omakorda jaguneda täisarvudeks (`integer`) ja murdarvudeks/ujukomaarvudeks (`double`). Murdarvus kasutatakse R-is vaikimisi koma asemel punkti (nt `2.7`, mitte `2,7`)!  
...
<br>
Vahel kodeeritakse andmestikes tundmatud väärtused mingi kindla väärtuse abil, nt `-99`, `.` vmt. Sel juhul ei tea R vaikimisi, et tegemist on erilise väärtusega, ning seda tuleb andmeid sisse lugedes talle eraldi öelda.   
...
<br>
<p style="text-align: center;">![](imgs/r_data_structures.png){width=70%}</p>
<p style="text-align: center;">R-i objektiklassid (https://web.itu.edu.tr/~tokerem/r_language.html)</p>
<br>
...
**Proovi nüüd loodud vektorite peal funktsiooni `length()`, mis ütleb, mitu väärtust vektoris on.**     
...
Kui andmestik on R-is korrastatud ja **RData** formaadis salvestatud, siis neid andmestikke saab avada käsuga **`load()`**. Näiteks sellisel kujul on Moodle'is failis *kysimustik_2026.RData* korrastatud vastused küsimustikule, mida eelmises praktikumis täitsite. Lisaks on seal ka varasemate aastate vastused. Laadige RData andmestik alla ja pange see oma kursuse andmestike kausta (nt kausta nimega `andmestikud`, `andmed`, `data` vmt), **muutke vastavalt ära ka faili teekonna nimi** (nt `andmestikud/kysimustik_2026.RData`, `andmed/kysimustik_2026.RData`, `data/kysimustik_2026.RData`).
...
Kui andmetabel ei ole RData-formaadis (enamasti ei ole), saab selle sisselugemiseks kasutada paljusid muid funktsioone, mis algavad üldjuhul sõnaga **`read`**.   

**csv**- või **txt**-formaadis tabeleid saab lugeda näiteks järgmiste käskudega, mis erinevad põhiliselt selle poolest, millised on erinevate argumentide (nt `header`, `sep`) vaikeväärtused:     
...
kysimustik_csv <- read.csv2("data/kysimustik_2026.csv", sep = "\t", encoding = "UTF-8")
```

Failil `kysimustik_2026.csv` on tulbanimedega päis (`header`), tulpasid eraldavad tabulaatorid (`\t`) ning faili kodeering on UTF-8. Kuna funktsioonide vaikeväärtused on erinevad (näiteks `read.table()` eeldab vaikimisi, et tabelil ei ole tulbanimesid), peame eri funktsioonidega andmeid sisse lugedes täpsustama erinevaid asju, vastavalt oma andmestiku struktuurile.  
...
library(readxl)
kysimustik_xlsx <- read_excel("data/kysimustik_2026.xlsx")
```
...
kysimustik_dta <- read_stata("data/kysimustik_2026.dta") # STATA
```
...
library(jsonlite)
kysimustik_json <- fromJSON("data/kysimustik_2026.json")
```
...
library(xml2)
kysimustik_xml <- as.data.frame(read_xml("data/kysimustik_2026.xml"))
```
...
**Ridade indekseid täpsustame kantsulgude sees enne koma ja tulpade indekseid pärast koma.**   
...
```{r}
# tabel
kysimustik[kysimustik$synniaasta > 2002,] # read, kus sünniaasta väärtus on suurem kui 2002
...
Arvulised andmed ja nende visualiseerimine.
